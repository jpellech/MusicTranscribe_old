...
def process_file():
    ...
    subprocess.run(["poetry", "run", "python", os.path.abspath(directory_path + "/MusicAssist.py"), os.path.abspath(directory_path + "/inputs") + "/" + file.filename])

    # Check if the output folder exists
    if os.path.exists(output_folder):
        zip_filename = f"{os.path.splitext(file.filename)[0]}.zip"
        
        # Adjust the path to save and check for the zip file in directory_path
        zip_filepath = os.path.join(directory_path, zip_filename)
        
        # Check if the zip file exists in the correct directory
        if os.path.exists(zip_filepath):
            # Prepare the zip file for download
            return send_from_directory(directory_path, zip_filename, as_attachment=True)
        else:
            return f"Zip file not found in the correct directory: {zip_filepath}", 404
    else:
